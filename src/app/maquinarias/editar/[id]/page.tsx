...
import { Save, Truck, Wrench, X, MapPin, Fuel, Settings, CreditCard, Calendar, Activity, DollarSign } from 'lucide-react'
...
export default function EditarMaquinariaPage({ params }: { params: { id: string } }) {
  ...
  const { machinery, updateMachinery } = useApp()
  const machineryId = parseInt(params.id)
  const currentMachinery = machinery.find(m => m.id === machineryId)
  ...
  useEffect(() => {
    if (currentMachinery) {
      reset({
        patent: currentMachinery.patent,
        type: currentMachinery.type,
        brand: currentMachinery.brand,
        model: currentMachinery.model,
        year: currentMachinery.year,
        total_hours: currentMachinery.total_hours,
        status: currentMachinery.status,
        fuel_capacity: currentMachinery.fuel_capacity,
        hourly_cost: currentMachinery.hourly_cost,
        last_location: currentMachinery.last_location,
        images: currentMachinery.images || [],
      })
      // Inicializar imágenes existentes - crear una copia profunda para evitar referencias
      if (currentMachinery.images && currentMachinery.images.length > 0) {
        setImages(JSON.parse(JSON.stringify(currentMachinery.images)))
      } else {
        setImages([])
      }
    } else {
  ...
  }, [currentMachinery, reset, router])

  const onSubmit = (data: MachineryFormData) => {
    if (!currentMachinery) return

    // Crear una copia profunda de las imágenes para asegurar que se guarden correctamente
    const imagesToSave = Array.isArray(images) && images.length > 0
    ...
    updateMachinery(machineryId, updatedMachinery)
  ...
  if (!currentMachinery) {
    return (
      <div className="p-6 text-center text-gray-600">Cargando o maquinaria no encontrada...</div>
    )
  }
        ...
        <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-2xl shadow-xl">
          ...
          <div className="relative p-8 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-white mb-1">Editar Maquinaria: {currentMachinery.brand} {currentMachinery.model}</h1>
                <p className="text-blue-100">Actualiza la información de la maquinaria</p>
              </div>
            </div>
            <Button 
              variant="outline" 
