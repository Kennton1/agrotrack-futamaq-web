...
import { Plus, Wrench, Calendar, User, DollarSign, X, Trash2, Edit, Truck, Settings, Clock, FileText, Package, AlertCircle, Save, Activity } from 'lucide-react'
...
export default function EditarMantenimientoPage() {
  ...
  const { updateMaintenance, maintenances, machinery } = useApp()
  ...
  const maintenanceId = params?.id ? parseInt(params.id as string) : null
  const currentMaintenance = maintenances.find(m => m.id === maintenanceId)
  ...
  useEffect(() => {
    if (currentMaintenance) {
      reset({
        machinery_id: currentMaintenance.machinery_id,
        type: currentMaintenance.type,
        status: currentMaintenance.status,
        scheduled_date: currentMaintenance.scheduled_date.split('T')[0],
        completion_date: currentMaintenance.completion_date ? currentMaintenance.completion_date.split('T')[0] : '',
        description: currentMaintenance.description,
        technician: currentMaintenance.technician,
        odometer_hours: currentMaintenance.odometer_hours,
      })

      // Cargar items de mantenimiento
      if (currentMaintenance.items && currentMaintenance.items.length > 0) {
        setItems(currentMaintenance.items.map(item => ({
          id: item.id,
          name: item.name,
      ...
      setLoading(false)
    } else if (maintenanceId && maintenances.length > 0) {
      // Si no se encuentra el mantenimiento pero hay mantenimientos cargados
      toast.error('Mantenimiento no encontrado')
      router.push('/mantenimientos')
    }
  }, [currentMaintenance, maintenanceId, maintenances, reset, router])
  ...
  const onSubmit = (data: MaintenanceFormData) => {
    ...
    const maintenanceItems: MaintenanceItem[] = items.map(item => ({
      ...
      notes: item.notes,
      parts_required: item.parts_required,
      scheduled_date: item.scheduled_date || data.scheduled_date,
      assigned_technician: item.assigned_technician || data.technician,
    }))
      ...
      <div className="p-6">
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">Mantenimiento no encontrado</h3>
            <p className="text-gray-500 mb-4">El mantenimiento que buscas no existe o ha sido eliminado.</p>
            <Button onClick={() => router.push('/mantenimientos')}>
        ...
        <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-2xl shadow-xl">
          ...
          <div className="relative p-8 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-white mb-1">Editar Mantenimiento</h1>
                <p className="text-blue-100">Modifica la información del mantenimiento {currentMaintenance.machinery_code}</p>
              </div>
            </div>
            <Button 
              variant="outline" 
                              ...
                              <div className="flex items-center gap-2 mb-2 flex-wrap">
                                ...
                                <Badge 
                                  variant={
                                    ...
                                    item.status === 'en_progreso' ? 'info' : 'default'
                                  } 
                                  size="sm"
                                >
                                  {item.status}
                              ...
                              <div className="flex items-center gap-4 text-sm flex-wrap">
                                ...
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span className="font-medium text-gray-900">
                                    ...
                                    {item.actual_hours && <span className="text-gray-400"> ({item.actual_hours}h reales)</span>}
                                  </span>
                                </div>
                              </div>
                              {item.notes && (
      ...
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => 
                  setFormData({ ...formData, status: value as MaintenanceItemForm['status'] })
                }
              >
                <SelectTrigger>
                  <SelectValue />
          ...
          {formData.status === 'completado' && (
            <div>
              <Label htmlFor="item_actual_hours">Horas Reales</Label>
              <Input
                id="item_actual_hours"
                type="number"
                value={formData.actual_hours || ''}
                onChange={(e) => setFormData({ ...formData, actual_hours: parseFloat(e.target.value) || undefined })}
                placeholder="0"
                min="0"
                step="0.5"
              />
