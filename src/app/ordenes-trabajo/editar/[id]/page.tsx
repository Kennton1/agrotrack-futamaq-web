...
import { Save, ClipboardList, Calendar, User, MapPin, X, Building2, Wrench, AlertCircle, Truck, FileText, Target, Activity } from 'lucide-react'
...
const workOrderSchema = z.object({
  ...
  status: z.enum(['planificada', 'en_ejecucion', 'completada', 'retrasada', 'detenida', 'cancelada'], {
  ...
  actual_hectares: z.number().min(0, 'Las hectáreas reales no pueden ser negativas'),
  progress_percentage: z.number().min(0).max(100, 'El porcentaje de progreso debe estar entre 0 y 100'),
})

type WorkOrderFormData = z.infer<typeof workOrderSchema>

export default function EditarOrdenTrabajoPage({ params }: { params: { id: string } }) {
  const router = useRouter()
  const { workOrders, updateWorkOrder, machinery } = useApp()
  ...
  useEffect(() => {
    if (currentWorkOrder) {
      reset({
        client_id: currentWorkOrder.client_id,
        field_name: currentWorkOrder.field_name,
        task_type: currentWorkOrder.task_type,
        description: currentWorkOrder.description,
        priority: currentWorkOrder.priority,
        planned_start_date: currentWorkOrder.planned_start_date,
        planned_end_date: currentWorkOrder.planned_end_date,
        actual_start_date: currentWorkOrder.actual_start_date || '',
        actual_end_date: currentWorkOrder.actual_end_date || '',
        status: currentWorkOrder.status,
        assigned_machinery: currentWorkOrder.assigned_machinery,
        target_hectares: currentWorkOrder.target_hectares,
        actual_hectares: currentWorkOrder.actual_hectares,
        progress_percentage: currentWorkOrder.progress_percentage,
      })
    } else {
      toast.error('Orden de trabajo no encontrada.')
      router.push('/ordenes-trabajo')
    }
  }, [currentWorkOrder, reset, router])
  ...
  const onSubmit = (data: WorkOrderFormData) => {
    ...
    const updatedWorkOrder = {
      ...
      target_hours: 0,
      actual_hectares: data.actual_hectares,
      actual_hours: currentWorkOrder.actual_hours,
      progress_percentage: data.progress_percentage,
      updated_at: new Date().toISOString(),
    }

    // Actualizar en el contexto
    updateWorkOrder(currentWorkOrder.id, updatedWorkOrder)
    router.push('/ordenes-trabajo')
  }

  const selectedMachineryIds = watch('assigned_machinery')
  const selectedMachinery = machinery.filter(m => selectedMachineryIds.includes(m.id))

  if (!currentWorkOrder) {
    return (
      <div className="p-6 text-center text-gray-600">Cargando o orden de trabajo no encontrada...</div>
    )
  }
        ...
        <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-2xl shadow-xl">
          ...
          <div className="relative p-8 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-white mb-1">Editar Orden de Trabajo: {currentWorkOrder.id}</h1>
                <p className="text-blue-100">Actualiza la información de la orden de trabajo</p>
              </div>
            </div>
            <Button 
              variant="outline" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Controller
                      ...
                      control={control}
                      render={({ field }) => {
                        const clientLabels: { [key: string]: string } = {
                          ...
                          <Select onValueChange={(value) => field.onChange(parseInt(value))} value={field.value.toString()}>
                            <SelectTrigger className={`h-11 ${errors.client_id ? 'border-red-500 focus:border-red-500' : 'border-gray-300 focus:border-blue-500 focus:ring-blue-500'}`}>
                              <span className={field.value ? "text-gray-900" : "text-gray-500"}>
                                {field.value ? clientLabels[field.value.toString()] : "Selecciona un cliente"}
                              </span>
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="1">Agrícola San Antonio S.A.</SelectItem>
                              <SelectItem value="2">Fundo El Carmen</SelectItem>
                              <SelectItem value="3">Cooperativa Agrícola Los Ríos</SelectItem>
                              <SelectItem value="4">Hacienda Santa Rosa</SelectItem>
                              <SelectItem value="5">Agroindustria del Sur</SelectItem>
                            </SelectContent>
                          </Select>
                        )
                      }}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Input
                        ...
                        className={`h-11 pl-10 ${errors.task_type ? 'border-red-500 focus:border-red-500' : 'border-gray-300 focus:border-blue-500 focus:ring-blue-500'}`}
                      />
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="actual_start_date" className="flex items-center space-x-2 text-gray-700 font-medium">
                      <Calendar className="h-4 w-4 text-blue-600" />
                  ...
                  {/* Fecha de Fin Real */}
                  <div className="space-y-2">
                    <Label htmlFor="actual_end_date" className="flex items-center space-x-2 text-gray-700 font-medium">
                      <Calendar className="h-4 w-4 text-blue-600" />
                  ...
                  {/* Hectáreas Reales */}
                  <div className="space-y-2">
                    <Label htmlFor="actual_hectares" className="flex items-center space-x-2 text-gray-700 font-medium">
                      <Target className="h-4 w-4 text-blue-600" />
                    ...
                    <div className="relative">
                      <Input
                        id="actual_hectares"
                        type="number"
                        {...register('actual_hectares', { valueAsNumber: true })}
                        placeholder="Ej: 28"
                        className={`h-11 pl-10 ${errors.actual_hectares ? 'border-red-500 focus:border-red-500' : 'border-gray-300 focus:border-blue-500 focus:ring-blue-500'}`}
                      />
                      <Target className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
                    </div>
                    {errors.actual_hectares && <p className="text-red-500 text-sm mt-1">{errors.actual_hectares.message}</p>}
                  </div>

                  {/* Progreso */}
                  <div className="space-y-2">
                    <Label htmlFor="progress_percentage" className="flex items-center space-x-2 text-gray-700 font-medium">
                      <Activity className="h-4 w-4 text-blue-600" />
                      <span>Progreso (%)</span>
                    </Label>
                    <div className="relative">
                      <Input
                        ...
                        placeholder="Ej: 62"
                        className={`h-11 pl-10 ${errors.progress_percentage ? 'border-red-500 focus:border-red-500' : 'border-gray-300 focus:border-blue-500 focus:ring-blue-500'}`}
                      />
                      <Activity className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
                    </div>
                    {errors.progress_percentage && <p className="text-red-500 text-sm mt-1">{errors.progress_percentage.message}</p>}
                  </div>
                      ...
                      {machinery.map((mach) => (
                        <label key={mach.id} className="flex items-center space-x-2 p-3 border border-gray-300 rounded-lg hover:bg-gray-50 hover:border-blue-500 cursor-pointer transition-colors">
                          <input
                            type="checkbox"
                            value={mach.id}
                            checked={selectedMachineryIds.includes(mach.id)}
                            onChange={(e) => {
                              const value = parseInt(e.target.value)
                              const current = watch('assigned_machinery')
                              if (e.target.checked) {
                                setValue('assigned_machinery', [...current, value])
                              } else {
                                setValue('assigned_machinery', current.filter(id => id !== value))
                              }
                            }}
                ...
                <div className="relative p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-gradient-to-br from-blue-600 to-indigo-600 rounded-lg p-4 text-white shadow-lg">
                      ...
                      <p className="text-2xl font-bold">{progress || 0}%</p>
                    </div>
                  </div>
                </div>
              </div>
