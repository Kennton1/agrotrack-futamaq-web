...
import { Save, Fuel, X, MapPin, DollarSign, Calendar, Truck, User, Package, FileText, Camera } from 'lucide-react'
...
export default function EditarCargaCombustiblePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (currentFuelLoad) {
      reset({
        machinery_id: currentFuelLoad.machinery_id,
        operator_id: currentFuelLoad.operator_id,
        date: currentFuelLoad.date,
        liters: currentFuelLoad.liters,
        cost_per_liter: currentFuelLoad.cost_per_liter,
        work_order_id: currentFuelLoad.work_order_id || '',
  ...
  }, [currentFuelLoad, reset, router])

  const onSubmit = (data: FuelLoadFormData) => {
    if (!currentFuelLoad) return
  ...
  if (!currentFuelLoad) {
    return (
      <div className="p-6 text-center text-gray-600">Cargando o carga de combustible no encontrada...</div>
    )
  }
        ...
        <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-2xl shadow-xl">
          ...
          <div className="relative p-8 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-white mb-1">Editar Carga de Combustible</h1>
